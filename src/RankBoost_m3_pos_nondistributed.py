...
class RankBoost_m3_pos(object):
	...
	def fit(self, X_bags, y_labels):
		...
		for index_Boosting in range(max_iter_boosting):
			...
			#predictions = sigmoid(instance_classifier.predict(instances) )
			predictions = instance_classifier.predict_proba(instances)[:,1] 

			self.r.append(  compute_r(np.array(instance_labels_generated_from_bag_labels), np.array(predictions), np.array(weights_inst) )  )
			self.alphas.append(   0.5*np.log(  (float(1)+self.r[-1])/(1-self.r[-1])  )   )
			...
			for inst_index in range(num_instances):
				if instance_labels_generated_from_bag_labels[inst_index]==1:
					weights_inst[inst_index] = weights_inst[inst_index]*np.exp(-self.alphas[-1]*predictions[inst_index])
					Z["positive"] += weights_inst[inst_index]
				else:
					weights_inst[inst_index] = weights_inst[inst_index]*np.exp(+self.alphas[-1]*predictions[inst_index])
					Z["negative"] += weights_inst[inst_index]
			for inst_index in range(num_instances):
				if instance_labels_generated_from_bag_labels[inst_index]==1:
					weights_inst[inst_index] /= float(Z["positive"])
				else:
					weights_inst[inst_index] /= float(Z["negative"] )
			#import pdb;pdb.set_trace()
	...
	def predict(self, X_bags, iter = None):
		...
		if type(X_bags) != list:  # treat it as normal supervised learning setting
			...
			predictions_list = [ instance_classifier.predict_proba(X_bags)[:,1].reshape((1, -1))  for instance_classifier in self.weak_classifiers ]
			#import pdb;pdb.set_trace()
			predictions_accum = np.matrix(self.c[0:iter])*np.matrix( np.vstack((predictions_list[0:iter])) )/np.sum(self.c[0:iter])

			#import pdb;pdb.set_trace()
			#return (np.array(predictions_accum)[0] - threshold)>0 + 0   #binary output: we need to deduct a threshold because instance_classifier.predict_proba is within range [0, 1]
			return np.array(predictions_accum)[0] - threshold   #output in range [-0.5, 0.5]: we need to deduct a threshold because instance_classifier.predict_proba is within range [0, 1]
		else:
...
def compute_r(labels, predictions, weights):
	"""
	predictions are in the range [0, 1]
	positive labels are represented by 1 and negative labels are represented otherwise
	weights are real values

	all of them are in the form of one-dimensional np.array and the order are consistent with each other

	the following computatations are based on Eq. (8) from Freund's original rankboost paper 'An Efficient Boosting Algorithm for Combining Preferences'
	"""
	num_inst = labels.shape[0]
	s = 2*(labels == 1) - 1 #+1 for positive instances, -1 for negative instances
	weight_sum = {}
	weight_sum[1] = np.sum(weights[s == 1])
	weight_sum[-1] = np.sum(weights[s == - 1])
	
	d =weights * np.array([ weight_sum[- labels[index] ]   for index in range(num_inst)  ])
	
	r = np.sum(d*s*predictions)
	return r
