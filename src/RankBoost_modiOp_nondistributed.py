...
Here, we assume alpha = 0.5*log( (epsilon^+ + (1-beta)*epsilon^0)/( epsilon^- + beta*epsilon^0 )   )
When epsilon^- != 0, the optimal beta will make alpha = 0.5*log( (epsilon^+ )/( epsilon^- )   ), i.e. the same in original rankboost
When epsilon^- = 0, the optimal beta should be 0, which will make alpha infinite. Instead, we make beta to be a very small positive number
"""
	...
	def fit(self, X_bags, y_labels):
		...
		for index_Boosting in range(max_iter_boosting):
			...
			if self.epsilon["negative"][-1] == 0 and self.epsilon["zero"][-1] == 0:
				...
				break
			elif self.epsilon_pair_fast["negative"][-1] == 0:
				self.alphas.append(0.5*np.log(  (self.epsilon_pair_fast["positive"][-1]+(1-self.beta)*self.epsilon_pair_fast["zero"][-1])/(self.epsilon_pair_fast["negative"][-1]+self.beta*self.epsilon_pair_fast["zero"][-1])  ))
			else:
