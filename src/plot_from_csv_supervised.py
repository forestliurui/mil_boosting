...
def generateRank(directory, outputfile_name):
	...
	method_names_prechosen = set(["rankboost","adaboost","martiboost","miboosting_xu", "rankboost_modiII", "rankboost_modiOp"])
...
def draw_plot1(directory, outputfile_name):
	...
	colors={'rankboost':'r', 'rankboost_modiOp':'b','adaboost':'k', 'martiboost':'c', 'rankboost_pos':'y','rankboost_m3':'m', 'rankboost_modiII':'g' }
	...
	for dataset_name in dataset_names:
		...
		for stat_index in range(len(statistics_name)):
			...
			if stat_name == 'test_instance_best_balanced_accuracy':
				...
				plt.ylabel('test_AUC', fontsize = 60)
			elif stat_name == 'train_instance_AUC':
				plt.ylabel('train_AUC', fontsize = 60)
			else:
