...
def generateAppendixLikeTable(directory, outputfile_name):
	...
	#stat_caption_map = {'test_error': 'Test Ranking Error'}
        ...
        boosting_round = 100
        statistics_name = ['test_error', 'test_error_tied']                       
        stat_caption_map = {'test_error': 'Test $\hat{R}_1$', 'test_error_tied': 'Test $\hat{R}_2$'}
	...
	method_names_prechosen = set(["rankboost","adaboost","martiboost","miboosting_xu", "rankboost_modiII", "rankboost_modiIII", 'rboost', 'auerboost', 'rankboost_modiVI'])
	method_names = method_names.intersection(method_names_prechosen)

	methodname_map = {"adaboost": "\\RBA{}", "martiboost": "\\MB{}", "miboosting_xu":"\\MIB{}", "rboost":"\\rB{}", "auerboost":"\\AuerB{}", "rankboost_modiII": "\\RBB{}", "rankboost":"\\RBA{}","rankboost_modiIII":"\\CRB{}","rankboost_modiVI":"\\CRB{}" }

	#import pdb;pdb.set_trace()

        method_names = ['rankboost', 'rankboost_modiII', 'rankboost_modiVI']
...
def generateAppendixLikeTable1(directory, outputfile_name):
	"""
	generate the table used in appendix of latex files
	make the stat in the same table rather than having one table for each stat
        """
	...
	#for statistic in statistics_name:
		...
		line = "\\begin{tabular}{|"  
		line += "c|"*(len(method_names)*len(statistics_name)+1)
		line += "}\n"
		with open(outputfile_name, 'a+') as f:
			f.write(line)
		...
		if statistic not in ranks_average:
			ranks_average[statistic] = {}
                ...
                #write stat names
                line = "         &"
                line += (" &  ".join([ "\multicolumn{"+str(len(method_names))+"}{|c|}{"+ stat_caption_map[x]+"}"  for x in statistics_name]  )) 
                line += "\\\\ \n"
                with open(outputfile_name, 'a+') as f:
                        f.write(line)
...
def draw_plot_averaged(directory, outputfile_name):
	...
	# for modified rankboost
        ...
        statistics_name_nested_list = [['test_error', 'test_error_tied'], ['train_E_modi', 'train_E_vanilla'], ['train_error', 'train_error_tied']]
        #statistics_name = ['test_error', 'train_error']
        #statistics_name = ['test_error', 'train_error', 'train_E_modi', 'train_E_vanilla']
        statistics_name = set(reduce(lambda x, y: x+y, statistics_name_nested_list))
...
def draw_plot_averaged1(directory, outputfile_name):
	"""
	plot the error vs boosting round figures. Each figure corresponds to all methods on one dataset. Each method is one input file. 
	(The input is the same with the above generateRank())
	"""
	...
	# for modified rankboost
        ...
        colors={'rankboost':'b', 'rankboost_modiVII':'y','rankboost_modiVI':'r', 'rankboost_modiIII': 'c', 'rankboost_modiII': 'k'}
        #linestyles: rankboost_modiIII solid line, rankboost dotted line, rankboost_modiII dashed line
        #linestyles = {'rankboost_modiVI':'-', 'rankboost':(0,[10,10]),'rankboost_modiV':(0,[40,10]), 'rankboost_modiIII': (0, [40,10,10,10]) }
        #linestyles = {'rankboost_modiIII':'-', 'rankboost':'dotted','rankboost_modiV':'dashed', 'rankboost_modiVI':'-', 'rankboost_modiII': 'dashed' }
...
def draw_plot_averaged2(directory, outputfile_name):
	"""
	plot the error vs boosting round figures. Each figure corresponds to all methods on one dataset. Each method is one input file. 
	(The input is the same with the above generateRank())
	"""
	...
	# for modified rankboost
        ...
        statistics_name_nested_list = [['test_error', 'test_error_tied'] ,['train_E_vanilla', 'train_E_modi']]
        #statistics_name = ['test_error', 'train_error']
        #statistics_name = ['test_error', 'train_error', 'train_E_modi', 'train_E_vanilla']
        statistics_name = reduce(lambda x, y: x+y, statistics_name_nested_list)
	...
	output_name = 'ranking/' + outputfile_name
	#plt.figure(figsize=(14*len(statistics_name), 10*len(statistics_name)))
	plt.figure(figsize=(17*2, 14))
        for stat_index in range(len(statistics_name_nested_list)):        
               
	         #stat_name = statistics_name[stat_index]

		 #plt.subplot(4, math.ceil( len(statistics_name)/3), stat_index+1)
		 plt.subplot(1, 2, stat_index+1)
		 #plt.subplot(2, 1, stat_index+1)
		 plt.yticks(fontsize = 50)
		 plt.xticks(fontsize = 50)
		 plt.xlabel('Boosting Round', fontsize = 60)
...
def draw_plot_averaged3(directory, outputfile_name):
	"""
	plot the error vs boosting round figures. Each figure corresponds to all methods on one dataset. Each method is one input file. 
	(The input is the same with the above generateRank())
	"""
	...
	#subplot_handle = {}
        subplot_handle = []
        if len(outputfile_name.split(".")) >= 2:
            outputfile_name_base = ".".join(outputfile_name.split(".")[:-1])
            outputfile_name_ext = outputfile_name.split(".")[-1]
        else:
            outputfile_name_base = outputfile_name
            outputfile_name_ext = None
        #outputfile_name_base, outputfile_name_ext = outputfile_name.split(".")
	output_name = 'ranking/' + outputfile_name
	#plt.figure(figsize=(14*len(statistics_name), 10*len(statistics_name)))
	#plt.figure(figsize=(17*2, 14))
        for stat_index in range(len(statistics_name_nested_list)):        
                 output_name = "ranking/" + outputfile_name_base+ "_".join(statistics_name_nested_list[stat_index]) + "." + outputfile_name_ext 
	         #stat_name = statistics_name[stat_index]
...
def convertStatNames(names_arg):
...
def convertMethodNames(names_arg):
        if type(names_arg) is list:
            names = names_arg
        else:
            names = [names_arg]
	...
	for name in names:
		if name == 'rankboost_modiII':
			...
			#result.append('Rankboost')
			result.append('RB-D')
                elif name == 'rankboost_modiVI':
                        result.append('RankBoost+')
                elif name == 'rankboost_modiVII':
                        result.append('RankBoost+prime')
		else:
			result.append(name)
...
if __name__ == '__main__':
	...
	if func_invoked == 'plot':
        ...
        elif func_invoked == 'plotAvg1':
                draw_plot_averaged1(directory, outputfile_name)
        elif func_invoked == 'plotAvg2':
                draw_plot_averaged2(directory, outputfile_name)
	elif func_invoked == 'plotAvg3':
                draw_plot_averaged3(directory, outputfile_name)
